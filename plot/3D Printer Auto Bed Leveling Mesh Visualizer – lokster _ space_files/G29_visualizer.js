function G29_sample_data(target){
...
function G29_plot(source, target){
	jQuery(target).show();
		
	var raw_data = jQuery(source).val();
	var re = /^Recv:  /gmi;
	raw_data = raw_data.replaceAll(re, '');
	...
	Plotly.newPlot(jQuery(target).prop('id'), data, layout, {responsive: true});
	
}

jQuery(function() {
	G29_sample_data('#data');
});
